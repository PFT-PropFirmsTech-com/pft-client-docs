...
```mermaid
graph TD
    A[Create Account] -->|Instant| B[Welcome Email]
    A --> C[Purchase Challenge]
    C -->|Instant| D[Payment Success Email]
    C -->|Instant| E[New Trading Account Email]

    E --> F[Start Trading]
    F --> G{Trading Outcome}

    G -->|Pass Challenge| H[Challenge Passed Email]
    G -->|Break Rule| I[Challenge Not Passed Email]
    G -->|No Activity 15 days| J[Inactivity Warning Email]

    H --> K[Progress to Next Phase]
    K -->|Instant| L[Program Progression Email]
...
```mermaid
flowchart LR
    A[Register] -->|Welcome Email| B[Verify Email]
    B --> C[Purchase Challenge]
    C -->|Payment Success<br/>New Account Email| D[Receive MT5 Credentials]
    D --> E[Start Trading]
    E --> F{Meet Requirements?}
    F -->|Yes| G[Challenge Passed Email]
    F -->|No| H[Breach Email]
    G --> I[Progress to Next Phase]
...
```mermaid
flowchart TD
    A[Need to Withdraw] --> B[Submit KYC Documents]
    B -->|KYC Submitted Email| C[Wait for Review]
    C --> D{KYC Decision}
    D -->|Approved| E[KYC Approved Email]
    D -->|Rejected| F[KYC Rejected Email]
    F --> G[Fix Issues & Resubmit]
    G --> C
    E --> H[Sign Contract]
    H -->|Contract Pending Email| I[Wait for Review]
    I --> J{Contract Decision}
    J -->|Approved| K[Contract Approved Email]
    J -->|Rejected| L[Contract Rejected Email]
    L --> M[Fix & Resubmit]
    M --> I
    K --> N[Can Request Withdrawals]
```
